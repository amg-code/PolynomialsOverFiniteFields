...
class RichPolynomial:
    ...
    def _add_splitting_field(self):
        if not hasattr(self, "_RSplF"):
            if self.is_irreducible():
                self._RSplF = RichFiniteFieldClass.RichExtensionField(self._RFF, self.deg, "s", "S")
            else: 
                max_deg = LCM_list([fac.deg for fac in self.get_factors()])
                self._RSplF = RichFiniteFieldClass.RichExtensionField(self._RFF,max_deg, "b", "Y")            
        return
    ...
    def _add_factors(self):
        # TBD, Info: This changed so that multiplicity of factors is also considered
    ...
    def _add_all_roots(self):    
        self._all_roots = [(fac[_sage_const_0 ].list, (fac[_sage_const_0 ].get_splitting_field()).q, fac[_sage_const_1 ], (fac[_sage_const_0 ].get_RP_in_splitting_field()).roots()) for fac in self.get_factors()]
        return     
    ...
    # Checks if the polynomial is a composition of the form g(X^p)
    def is_composition(self, p):
        for i in [j for j in range(len(self.list)) if j%p !=_sage_const_0 ]:
    ...
    # If self is a composition of the form h^n*F(g/h), where F is a polynomial of known degree n and deg(h)<deg(g), then this function returns the polynomial F
    # For g,h such that gcd(g,h)=1
    def extract_from_composition(self, g, h):
        # TBD Check gcd(g,h)=1!

        FQ = RichPolynomial((self.list).copy(), self._RFF)
        F_lst = []

        # Algorithm for the case deg(g)>deg(h):
        if g.deg > h.deg:           

            while FQ.deg > _sage_const_0 :
                if FQ.deg % g.deg != _sage_const_0 :
                    raise ValueError("Polynomial cannot be a composition of the given rational function!") 
                a = FQ.list[_sage_const_0 ]/g.list[_sage_const_0 ]
                F_lst.append(a)
                FQ_new = FQ.get_polynomial()-a*(g.get_polynomial())**(FQ.deg/g.deg)
                print(FQ_new)
                if FQ_new != _sage_const_0 :
                    # TBD CHeck if this is really a check for being the zero-polynomial
                    while gcd(FQ_new, h.get_polynomial())==h.get_polynomial():
                        (q,r) = FQ_new.quo_rem(h.get_polynomial())
                        if r != _sage_const_0 :
                            raise ValueError("Something seriously went wrong, h no divisor.")
                        FQ_new = q

                    FQ_new = FQ_new.list()
                    FQ_new.reverse()
                    FQ = RichPolynomial(FQ_new,self._RFF)
